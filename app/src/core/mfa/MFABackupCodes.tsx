...
export default function MFABackupCodes({ codes, onClose }: Props) {
  ...
  return (
    <div className="unified-card mfa-backup-container" role="region" aria-label={t('backup_codes_aria_label')}>
      <div className="mfa-backup-warning" role="alert">
      ...
      <div className="mfa-backup-grid" role="list" aria-label={t('backup_codes_aria_label')}>
        {codes.map((code, i) => (
          <div key={i} className="mfa-backup-code" role="listitem" aria-label={t('backup_code_aria_label', { index: i + 1, total: codes.length })}>
            {code}
          </div>
        ))}
      </div>
